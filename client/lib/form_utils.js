	...
	formObject.find("input,select,textarea").each(function() {
		...
		if(!skipValue) {
			...
			if(inputObject.attr("required") && !fieldValue) {
				validationError(labelText + " es requerido");
				return false;
			}
			...
			if(dataType == "INTEGER") {
				if(fieldValue == "") {
					...
					if(isNaN(intValue)) {
						validationError(labelText + ": Valor no valido.");
						return false;
					}

					if(minValue && !isNaN(parseInt(minValue)) && intValue < parseInt(minValue)) {
						if(maxValue && !isNaN(parseInt(maxValue)))
							validationError(labelText + " debe ser entre " + minValue + " y " + maxValue);
						else
							validationError(labelText + " debe ser igual o mayor que " + minValue);
						return false;
					}
					...
					if(maxValue && !isNaN(parseInt(maxValue)) && intValue > parseInt(maxValue)) {
						if(minValue && !isNaN(parseInt(minValue)))
							...
							validationError(labelText + " debe ser igual o menor que " + maxValue);
						return false;
					}
					fieldValue = intValue;
				}
			...
			if(dataType == "EMAIL") {
				if(fieldValue && !isValidEmail(fieldValue)) {
					validationError(labelText + ": por favor digite una direccion de correo valida");
					return false;
			    }
			}
			...
			if(dataType == "IP") {
				if(fieldValue && !isValidIP(fieldValue)) {
					validationError(labelText + ": por favor digite una direccion IPv4 o IPv6 valida");
					return false;
				}
			}
			if(dataType == "IPV4") {
				if(fieldValue && !isValidIPv4(fieldValue)) {
					validationError(labelText + ": por favor digite una direccion IPv4 valida");
					return false;
				}
			}
			if(dataType == "IPV6") {
				if(fieldValue && !isValidIPv6(fieldValue)) {
					validationError(labelText + ": por favor digite una direccion IPv6 valida");
					return false;
				}
			}
			...
			if(dataType == "DATE") {
				if(fieldValue == "") {
					...
					if(!date.isValid()) {
						validationError(labelText + ": Valor no valido." + (dataFormat ? " La fecha debe ser en el formato \"" + dataFormat + "\"" : ""));
						return false;
					}
